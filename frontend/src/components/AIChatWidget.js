  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: 'bot',
      message: "👋 Hi! I'm your AI consultant assistant. How can I help you explore AI solutions for your business?",
      timestamp: new Date()
    }
  ]);
  ...
  const handleSendMessage = async (message = inputMessage) => {
    ...
    try {
      ...
      const errorMessage = {
        id: Date.now() + 1,
        type: 'bot',
        message: "I'm having trouble connecting right now. Please try again or contact us directly at hello@i10iq.ai",
        timestamp: new Date()
      };
                ...
                >
                  <div
                    className={`max-w-xs px-3 py-2 rounded-2xl ${
                      msg.type === 'user'
                        ? 'bg-golden text-white rounded-br-md'
            ...
            <div className="p-4 border-t border-gray-200">
              <div className="flex space-x-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-golden focus:border-transparent text-sm"
                />
                <button
                  onClick={() => handleSendMessage()}
                  disabled={!inputMessage.trim()}
                  className="bg-golden hover:bg-yellow-600 disabled:bg-gray-300 text-white p-2 rounded-full transition-colors"
                >
